...
void State::interpolate_course_x(int scheme, int index, int cell_lower, int cell_upper, double deltaT ){
   ...
   switch(scheme){
      case 0: // H,U,V interpolation, right cell more refined  
         ...
         cell_bot = mesh->nrht[cell_course];
         cell_top = mesh->ntop[cell_bot];

         dx_lower_bot = mesh->lev_deltax[mesh->level[cell_course]];
         dx_lower_top = mesh->lev_deltax[mesh->level[cell_course]];
         dx_upper_bot = mesh->lev_deltax[mesh->level[cell_bot]];
         dx_upper_top = mesh->lev_deltax[mesh->level[cell_top]];
      ...
      case 1: // H,U,V interpolation, left cell more refined  
         ...
         cell_bot = mesh->nlft[cell_course];
         cell_top = mesh->ntop[cell_bot];
         
         dx_lower_bot = mesh->lev_deltax[mesh->level[cell_bot]];
         dx_lower_top = mesh->lev_deltax[mesh->level[cell_top]];
         dx_upper_bot = mesh->lev_deltax[mesh->level[cell_course]];
         dx_upper_top = mesh->lev_deltax[mesh->level[cell_course]];

         real_t FA_lower_bot = dx_lower_bot;
         real_t FA_lower_top = dx_lower_top;
         real_t FA_upper_bot = dx_upper_bot;
         real_t FA_upper_top = dx_upper_top;
         real_t FA_lolim_bot = FA_lower_bot*min(ONE, FA_upper_bot/FA_lower_bot);
         real_t FA_lolim_top = FA_lower_top*min(ONE, FA_upper_top/FA_lower_top);
         real_t FA_uplim_bot = FA_upper_bot*min(ONE, FA_lower_bot/FA_upper_bot);
         real_t FA_uplim_top = FA_upper_top*min(ONE, FA_lower_top/FA_upper_top);

         real_t CV_lower_bot = SQ(dx_lower_bot);
         real_t CV_lower_top = SQ(dx_lower_top);
         real_t CV_upper_bot = SQ(dx_upper_bot);
         real_t CV_upper_top = SQ(dx_upper_top);
         real_t CV_lolim_bot = CV_lower_bot*min(HALF, CV_upper_bot/CV_lower_bot);
         real_t CV_lolim_top = CV_lower_top*min(HALF, CV_upper_top/CV_lower_top);
         real_t CV_uplim_bot = CV_upper_bot*min(HALF, CV_lower_bot/CV_upper_bot);
         real_t CV_uplim_top = CV_upper_top*min(HALF, CV_lower_top/CV_upper_top);
...
void State::interpolate_course_y(int scheme, int index, int cell_lower, int cell_upper , double deltaT ){
   ...
   switch(scheme){
      case 0: // H,U,V interpolation, top cell more refined  
         cell_course = cell_lower;
         cell_left = mesh->ntop[cell_course];
         cell_right = mesh->nrht[cell_left];

         dy_lower_left = mesh->lev_deltay[mesh->level[cell_course]];
         dy_lower_right = mesh->lev_deltay[mesh->level[cell_course]];
         dy_upper_left = mesh->lev_deltay[mesh->level[cell_left]];
         dy_upper_right = mesh->lev_deltay[mesh->level[cell_right]];
      ...
      case 1: // H,U,V interpolation, bottom cell more refined  
         cell_course = cell_upper;
         cell_left = mesh->nbot[cell_course];
         cell_right = mesh->nrht[cell_left];

         dy_lower_left = mesh->lev_deltay[mesh->level[cell_left]];
         dy_lower_right = mesh->lev_deltay[mesh->level[cell_right]];
         dy_upper_left = mesh->lev_deltay[mesh->level[cell_course]];
         dy_upper_right = mesh->lev_deltay[mesh->level[cell_course]];
